...
function handle(context: ServerContext, router: Router) {
    router.get("/blockNumber", async (_R, res, next) => {
        context.db
            .getLastBlockNumber()
            .then(n => {
                res.send(n.toString());
    ...
    router.get("/block/:blockNumber/hash", async (req, res, next) => {
        ...
        context.db
            .getBlock(Number.parseInt(blockNumber, 10))
            .then(hash => {
    ...
    router.get("/block/:id", async (req, res, next) => {
        ...
        try {
            ...
            const block = Type.isH256String(id)
                ...
                : await context.db.getBlock(Number.parseInt(id, 10));
    ...
    router.get("/blocks/totalCount", async (_R, res, next) => {
        try {
